...
public class AbstractIntegrationTest {
  ...
  private static final String HEXAGONAL_ARCHITECTURE_DB_NAME = "hexagonal-architecture";
  private static final DockerImageName MONGO_DB_IMAGE = DockerImageName.parse("mongo")
      .withTag("latest");
  public static final MongoDBContainer MONGO_DB_CONTAINER = new MongoDBContainer(MONGO_DB_IMAGE)
  ...
  public static DropwizardAppExtension<ApplicationConfiguration> EXTENDED_APPLICATION = new DropwizardAppExtension<>(
      FilmDropwizardApplication.class, null,
      ConfigOverride.config("mongodb.db", HEXAGONAL_ARCHITECTURE_DB_NAME),
      ConfigOverride.config("mongodb.uri",
          String.format("mongodb://localhost:%s/" + HEXAGONAL_ARCHITECTURE_DB_NAME,
              MONGO_DB_CONTAINER.getMappedPort(27017)))
  );

  @BeforeAll
  void setUpDb() throws Exception {
    EXTENDED_APPLICATION.before();

    Injector injector = Guice.createInjector(
        new MongoDbModule(EXTENDED_APPLICATION.getConfiguration()),
        new AdaptersModule(EXTENDED_APPLICATION.getConfiguration()));
    injector.injectMembers(this);

    MongoCollection<Document> collection = mongoClient.getDatabase(
            EXTENDED_APPLICATION.getConfiguration().getDb())
